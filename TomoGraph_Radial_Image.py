...
#-----------------------------------------------------------------------
    ...
    for iCol in range(0,imageWidth):  
        ...
        #print('topLayer.shape',topLayer.shape)
        #print('bottomLayer.shape',bottomLayer.shape)
...
def normal_graphCut(image,axialRes,laterRes,rpeTop,rpeBottom,invalidIndices,recalculateWeights):
    ...
    #for iLayer in range(0,1):
        ...
        if layerIndex == 2:
            ...
            innerLayer = np.round(savgol_filter(cut,151,2))
...
def normal_getGraphCutRegion(image, layersNumber, axialRes, eye,Index,Range, rpeTop, rpeBottom, layers):
    ...
    #cut OS-RPE
        ...
        a = savgol_filter(layers[3,:],151, 2)
...
inputImage = cv2.imread(dirName,0)
...
smoothing = np.array([151,151,151,749,749,749,749,749])
...
layer_Thickness = layer_PerImage[1] - layer_PerImage[0]

np.savetxt(sys.argv[2]+"/IlmRnflThickness.csv",layer_PerImage,delimiter=',',fmt='%d')
np.savetxt(sys.argv[2]+"/IlmRnflGraph.csv",layer_Thickness,delimiter=',',fmt='%d')

splitFour = np.array_split(layer_Thickness,4)
splitTwelve = np.array_split(layer_Thickness,12)

meanThickness = np.array([np.sum(layer_Thickness) / len(layer_Thickness)])
meanFour = np.zeros(len(splitFour))
meanTwelve = np.zeros(len(splitTwelve))

for i in range(len(splitFour)):
    meanFour[i] = np.sum(splitFour[i]) / len(splitFour[i])
    
for i in range(len(splitTwelve)):
    meanTwelve[i] = np.sum(splitTwelve[i]) / len(splitTwelve[i])

thicknessValues = np.concatenate([meanTwelve,meanFour,meanThickness])
thicknessValues = thicknessValues.reshape(1,len(thicknessValues))
np.savetxt(sys.argv[2]+"/ThicknessValues.csv",thicknessValues,delimiter=',',fmt='%f')